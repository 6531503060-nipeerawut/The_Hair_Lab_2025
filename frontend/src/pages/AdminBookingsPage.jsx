...
export default function AdminBookingsPage() {
  ...
  // --- State for Modals ---
  // Store the *entire* booking object to get ID and UserID
  const [resBooking, setResBooking] = useState(null);
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      console.error(err);
      setError("Failed to fetch bookings. See console for details.");
    }
    setLoading(false);
  };
  ...
  const changeStatus = async (booking, newStatus) => {
    try {
      ...
      await updateBooking(booking.userId, booking.id, { status: newStatus });
  ...
  const openReschedule = (booking) => {
    setResBooking(booking); // Store the whole booking object
    setResDate(booking.date); // Pre-fill modal
    setResTime(booking.time); // Pre-fill modal
    setResOpen(true);
  };
  ...
  const confirmReschedule = async () => {
    ...
    try {
      await updateBooking(resBooking.userId, resBooking.id, {
        date: resDate,
        time: resTime,
      });
      setResOpen(false);
  ...
  const openCancel = (booking) => {
    setCancelBooking(booking); // Store the whole booking object
    setCancelOpen(true);
  };
  ...
  const confirmCancel = async () => {
    ...
    try {
      ...
      await deleteBooking(cancelBooking.userId, cancelBooking.id);
      setCancelOpen(false);
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              {loading ? (
                  <tr>
                    <td colSpan="6" className="px-6 py-10 text-center text-gray-500">Loading bookings...</td>
                  </tr>
              ) : error ? (
                  <tr>
                  ...
                  filteredBookings.map((b, index) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <select
                              value={b.status}
                              onChange={(e) => changeStatus(b, e.target.value)} // Pass whole object
                        ...
                        <td className="px-6 py-4">
                          <div className="flex gap-2">
                            <button
                                onClick={() => openReschedule(b)} // Pass whole object
                            ...
                            <button
                                onClick={() => openCancel(b)} // Pass whole object
