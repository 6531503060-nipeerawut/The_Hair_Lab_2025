...
import { getBookings, revenueSum } from "../services/store";

export default function AdminDashboardPage() {
  const [bookings, setBookings] = useState([]); // ✅ เก็บข้อมูลจาก mock store
  const [loading, setLoading] = useState(true);

  // ✅ โหลดข้อมูลจาก mock store
  useEffect(() => {
    const fetchData = async () => {
      const data = await getBookings();
      setBookings(Array.isArray(data) ? data : []); // ป้องกันไม่ใช่ array
      setLoading(false);
    };
    fetchData();
  }, []);
